...
- **With tool retrieval on, a client that does not use Nest's MCP server gets a shortened tool list and is not told.** Retrieval selects the tools most relevant to the conversation and drops the rest, and the assistant is told its list is partial — and how to search for the rest — only when the `search_tools` tool is among the ones it received. Clients that connect to Nest's built-in MCP server (the chat UI, Twig) always get it. An OpenAI-compatible application that sends its own tool definitions instead gets the shorter list with no notice and no way to ask for what is missing, and may report a capability as unavailable when it is only unlisted. Retrieval is off by default; this applies once it is turned on.
