...
// =============================================================================
// Phase 1 pulled every IPC handler BODY out into a plain function. This is the
// next turn of the same screw: the BINDING comes out too. Each ipc/*.mjs module
// now exports a table — `{ 'llama:launch': (config) => ..., ... }` — and the two
// transports consume the same object:
//
...
// only platform HTTP-only exists for (plan decision 5). The table has to be the
...
// invariant scripts/test-ipc-contract.mjs pins for the preload bridge, which is
// what trap 5.8 asks for a replacement of.
//
 ...
 * pick a path on your laptop and save it as a path on the server (plan trap
 * 5.2). Phase 4 is what replaces these with a server-side browser and gates the
 * native one on "the daemon is local".
 *
