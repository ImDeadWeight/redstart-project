 ...
 * Prefers a logged-in session token over the legacy configured API key, so
 * a user who has logged in via the auth login form doesn't also need to
 * paste a key into settings — the dummy-apiKey fallback still works for
 * anyone who hasn't logged in and isn't required to (localhost, or auth off).
 */
