...
{:else}
	<div
		...
		role="main"
	>
		<!-- Scrollable area: fills all space above the form -->
		<div
			bind:this={chatScrollContainer}
			class="min-h-0 flex-1 overflow-y-auto px-4 md:px-6"
			onscroll={handleScroll}
		>
			{#if isEmpty}
				<!-- Greeting centered vertically in the scroll area -->
			...
			{:else}
				...
				<!-- Spacer so the last message clears the form -->
				<div class="h-4 shrink-0" aria-hidden="true"></div>
			{/if}
		</div>

		<!-- Form: always visible at the bottom, never scrolls away -->
		<div class="relative shrink-0 px-4 pb-4 pt-1 md:px-6">
			<ChatScreenActionScrollDown
