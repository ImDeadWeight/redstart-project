...
{#if items.length > 1}
	<div class="sticky bottom-0 z-10 mt-4 shrink-0">
		<HorizontalScrollCarousel class="max-w-full">
			{#each items as item, index (item.id)}
				<button
					data-thumbnail-index={index}
					class={[
						'relative shrink-0 cursor-pointer overflow-hidden rounded border-2 bg-black/80 backdrop-blur-sm transition-all hover:opacity-90',
