...
/** The configured capability root — what an admin picks in the Tools window. */
let outputDir: string;
/**
 * Where documents actually land: the caller's own folder inside that root.
 * These tests call the tool with no ctx, which is the auth-off case, so the
 * scope is the anonymous one. Asserting against the capability root would be
 * asserting the shared-storage behaviour that per-account scoping removed.
 */
...
describe('create_document — table rendering', () => {
	function tableMarkdown(rowCount: number) {
		...
		for (let i = 1; i <= rowCount; i++)
			md += `| 2026-01-${String(i).padStart(2, '0')} | Item ${i} | ${i} |\n`;
		return md;
	}
	...
	it('writes a real Word table, not pipe-text paragraphs', async () => {
		...
		const { value: html } = await mammoth.convertToHtml({
			path: join(writtenDir, 'purchase-log.docx')
		});
	...
	it('renders a table to PDF without error', async () => {
		...
		expect(result.isError).toBeFalsy();
		expect(existsSync(join(writtenDir, 'pdf-log.pdf'))).toBe(true);
	});
	...
	it('accepts filename as an alias for title and infers format from it', async () => {
		...
		expect(existsSync(join(writtenDir, 'tiling-company-purchase-log.docx'))).toBe(true);
	});
	...
	it('honors an explicit format over the filename extension', async () => {
		...
		expect(result.isError).toBeFalsy();
		expect(existsSync(join(writtenDir, 'report.md'))).toBe(true);
	});
	...
	it('ignores any directory in the supplied filename', async () => {
		...
		expect(existsSync(join(writtenDir, 'secret-notes.pdf'))).toBe(true);
	});
